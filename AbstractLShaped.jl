...
function init(lshaped::AbstractLShapedSolver)
    ...
    for i = 1:n
        lshaped.subProblems[i] = SubProblem(getchildren(m)[i],lshaped.structuredModel,i,π[i])
    end
...
function prepareMaster!(lshaped::AbstractLShapedSolver,n)
    ...
    updateObjective!(lshaped::AbstractLShapedSolver)
...
function updateObjective!(lshaped::AbstractLShapedSolver)
...
function addCut!(lshaped::AbstractLShapedSolver,subprob::SubProblem)
    ...
    if substatus == :Optimal
        ...
        w = e-E⋅x
        θ = lshaped.ready[subprob.id] ? getvalue(lshaped.θs[subprob.id]) : -Inf
        τ = lshaped.τ

        lshaped.ready[subprob.id] = true
