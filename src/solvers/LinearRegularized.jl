@with_kw mutable struct LinearRegularizedData{T <: Real}
    Q::T = 1e10
    Q̃::T = 1e10
    θ::T = -1e10
    σ::T = 1.0
...
@with_kw mutable struct LinearRegularizedParameters{T <: Real}
    τ::T = 1e-6
    σ::T = 1.0
    σ̅::T = 4.0
    σ̲::T = 0.5
...
struct LinearRegularized{T <: Real, A <: AbstractVector, M <: LQSolver, S <: LQSolver} <: AbstractLShapedSolver{T,A,M,S}
    structuredmodel::JuMP.Model
    solverdata::LinearRegularizedData{T}
    ...
    # Params
    parameters::LinearRegularizedParameters{T}
    progress::ProgressThresh{T}

    @implement_trait LinearRegularized IsRegularized
    @implement_trait LinearRegularized LinearizedQuadraticPenalty

    function (::Type{LinearRegularized})(model::JuMP.Model,ξ₀::AbstractVector,mastersolver::AbstractMathProgSolver,subsolver::AbstractMathProgSolver; kw...)
        if nworkers() > 1
        ...
        lshaped = new{T,A,M,S}(model,
                               ...
                               Vector{SparseHyperPlane{T}}(),
                               A(),
                               LinearRegularizedParameters{T}(;kw...),
...
LinearRegularized(model::JuMP.Model,mastersolver::AbstractMathProgSolver,subsolver::AbstractMathProgSolver; kw...) = LinearRegularized(model,rand(model.numCols),mastersolver,subsolver; kw...)

function (lshaped::LinearRegularized)()
    # Reset timer
