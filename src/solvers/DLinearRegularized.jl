@with_kw mutable struct DLinearRegularizedData{T <: Real}
    Q::T = 1e10
    Q̃::T = 1e10
    θ::T = -1e10
    σ::T = 1.0
...
@with_kw mutable struct DLinearRegularizedParameters{T <: Real}
    κ::T = 0.3
    τ::T = 1e-6
    σ::T = 1.0
    σ̅::T = 4.0
...
struct DLinearRegularized{T <: Real, A <: AbstractVector, M <: LQSolver, S <: LQSolver} <: AbstractLShapedSolver{T,A,M,S}
    structuredmodel::JuMP.Model
    solverdata::DLinearRegularizedData{T}
    ...
    # Params
    parameters::DLinearRegularizedParameters{T}
    progress::ProgressThresh{T}

    @implement_trait DLinearRegularized IsRegularized
    @implement_trait DLinearRegularized IsParallel
    @implement_trait DLinearRegularized LinearizedQuadraticPenalty

    function (::Type{DLinearRegularized})(model::JuMP.Model,ξ₀::AbstractVector,mastersolver::AbstractMathProgSolver,subsolver::AbstractMathProgSolver; kw...)
        if nworkers() == 1
            warn("There are no worker processes, defaulting to serial version of algorithm")
            return LinearRegularized(model,ξ₀,mastersolver,subsolver; kw...)
        end
        ...
        lshaped = new{T,A,M,S}(model,
                               ...
                               Vector{SparseHyperPlane{T}}(),
                               A(),
                               DLinearRegularizedParameters{T}(;kw...),
...
DLinearRegularized(model::JuMP.Model,mastersolver::AbstractMathProgSolver,subsolver::AbstractMathProgSolver; kw...) = DLinearRegularized(model,rand(model.numCols),mastersolver,subsolver; kw...)

function (lshaped::DLinearRegularized)()
    # Reset timer
